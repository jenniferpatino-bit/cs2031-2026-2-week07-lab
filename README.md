...
## Cómo ejecutar esta solución

```bash
cp .env.example .env          # ajusta DB_PORT / JWT_SECRET si hace falta
docker compose up -d          # PostgreSQL (puerto ${DB_PORT})
./mvnw spring-boot:run        # API en http://localhost:8080 (usa SERVER_PORT=8081 si el 8080 está ocupado)
```

| Variable | Descripción |
|---|---|
| `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD` | Conexión a PostgreSQL (el `compose.yaml` usa los mismos valores). |
| `JWT_SECRET` | Clave HS256, mínimo 32 bytes (`openssl rand -hex 32`). |
| `JWT_EXPIRATION_ACCESS` | Vigencia del token en ms (por defecto 3600000 = 1 h). |
| `BOOKING_EMAIL_DIR` | Carpeta donde se escriben los `flight_booking_email_${booking_id}.txt` (por defecto `emails/`). |

Endpoints: `POST /flights/create`, `POST /users/register`, `POST /auth/login` (públicos); `GET /flights/search?flightNumber=&airline=&departureFrom=&departureTo=`, `POST /flights/book`, `GET /flight/book/{id}` (requieren `Authorization: Bearer <jwt>`).
